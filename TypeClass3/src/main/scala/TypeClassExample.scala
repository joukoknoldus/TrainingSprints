object TypeClassExample {
  def main(args: Array[String]): Unit =
  {
    ...
    val optimizedVdwDistance=calcObject.optimizeDistance(vdw.calcU)
    ...
    val optimizedBuckDistance=calcObject2.optimizeDistance(buck.calcU)
...
class Calc[A <: TwoAtoms](atomPair: A) {
  ...
  def optimizeDistanceRecursive[A <: TwoAtoms](newAtomPair: A, oldU: Double, stepSize: Double, maxIterations: Int, calcUFunc: () => Double): Double = {
    ...
    if (u<oldU) {
      ...
      optimizeDistanceRecursive(newAtomPair2, u, stepSize*1.1, maxIterations-1, calcUFunc)
    }
    else {
      val newAtomPair2 = atomPair.copyAtomPairWithNewDistance(newAtomPair.distance - stepSize*1.5)
      optimizeDistanceRecursive(newAtomPair2, oldU, -stepSize*0.5, maxIterations-1, calcUFunc)
    }
  }

  def optimizeDistance(calcUFunc: () => Double): Double = {
    ...
    optimizeDistanceRecursive(newAtomPair, oldU, stepSize, maxIterations, calcUFunc)
  }
}
